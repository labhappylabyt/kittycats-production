...
export function SoundToggle() {
  ...
  return (
    <button
      ...
      className="group fixed right-4 top-20 z-50 flex h-11 items-center gap-2 overflow-hidden rounded-[12px] border border-white/15 bg-[#222629] px-3 text-white/70 shadow-[4px_4px_0_rgba(0,0,0,.2)] transition-all duration-300 hover:-translate-y-0.5 hover:border-[#d9f27c]/60 hover:text-[#d9f27c] sm:right-7 sm:top-20"
    >
      <span className={`h-1.5 w-1.5 rounded-full transition-colors ${muted ? 'bg-white/25' : 'bg-[#d9f27c]'}`} />
      {muted ? (
