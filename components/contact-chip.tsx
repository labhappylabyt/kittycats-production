...
export function ContactChip({ email }: { email: string }) {
  ...
  return (
    <div className="relative">
      <button type="button" onClick={copy} className="group inline-flex items-center gap-3 rounded-[14px] border border-white/15 bg-[#222629] px-4 py-3 text-sm text-white shadow-[4px_4px_0_rgba(0,0,0,.2)] transition-all duration-300 hover:-translate-y-1 hover:border-[#d9f27c]/60 hover:bg-[#d9f27c] hover:text-[#191a1d] active:translate-y-0 active:shadow-[2px_2px_0_rgba(0,0,0,.2)]" aria-label={`Copy email ${email}`}>
      ...
      <div className={`pointer-events-none absolute -top-9 right-0 whitespace-nowrap rounded-[8px] border border-[#d9f27c]/30 bg-[#222629] px-3 py-1.5 ui-label text-[#d9f27c] shadow-[3px_3px_0_rgba(0,0,0,.18)] transition-all duration-200 ${copied ? 'translate-y-0 opacity-100' : 'translate-y-1 opacity-0'}`}>Copied</div>
    </div>
  )
}
