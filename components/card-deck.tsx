...
// 3 copies of the deck, pre-mounted in one continuous flex row.
// The CSS @keyframes marquee translates the inner track from 0% to -33.333%
// (exactly one copy's width), then snaps back to 0% for a seamless loop.
// A separate wrapper handles drag/wheel offset so it never conflicts with
// the animation transform. Pure CSS animation = no React re-render lag.
const COPIES = 3
...
export function CardDeck() {
  ...
  // Decay drag offset back to 0 and apply it to the wrapper via rAF.
  useEffect(() => {
    let raf = 0
    const tick = () => {
